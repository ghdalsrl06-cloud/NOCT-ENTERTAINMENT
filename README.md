...
- 입체 렌더: `data/assets.json` 의 `logo3d`(심벌 1:1, 프로필용) · `logoLockup`(심벌 + NOCT + ENTERTAINMENT 16:9, 링크 미리보기 og:image) · `logoNoct`(NOCT 단독, C 자리에 달 마크, 16:9)
...
- [x] 3D(Three.js, `js/three-scenes.js`): 히어로 파편 다면체(스크롤 분해·재조립, 클릭 파동, 마우스 시차), 홈 전체 별먼지 필드, 지구본(대기 글로우·도시 라벨·드래그·위성), 스포트라이트 레코드판(CSS 3D)
